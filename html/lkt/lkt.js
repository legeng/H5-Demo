(function(w,d){
    ...
    var lkt = {
		...
		init:function(){
	    		if( lkt.Cookies.isSupportCookies() ){
				 var urlStr = lkt.urlParamStr();
				 var shortStr = lkt.urlParam();
				 var toDir = shortStr["url"];
				 decodeUrlStr = decodeURIComponent(toDir);
				 //要求传过来的url不要带utm_source
				 var lenBoolean = decodeUrlStr.indexOf("&") > -1;
				 if(lenBoolean){
				 	decodeUrlStr = decodeUrlStr +"&utm_source=cps_lkt";
				 }else{
				 	decodeUrlStr = decodeUrlStr +"?utm_source=cps_lkt";
				 }
			 	if(urlStr!=""){
			 		lkt.Cookies.setCookie("aihuishou_source_act",urlStr,15,"/","aihuishou.com");//lkt.Cookies.setCookie("sName",lkt_pram,0.02,"/","aihuishou.com");		 
			 	}
