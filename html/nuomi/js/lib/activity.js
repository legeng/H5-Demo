(function($){
    ...
    //检查输入值是否为空或格式错误
    Activity.prototype.checkValue = function($flag){
        if($(".phone").val() == ''){
            this.showBox('请输入手机号');
            return ;
        }
    ...
    Activity.prototype.getCode = function(){
        ...
        this.imgLogin(function(data){
            if(data.code == 0){
                ...
                //data={"code":"-1","msg":"发送验证码controller异常","body":null}
                clearInterval(this.option.timer);
                this.option.smsClicked = 1;
                this.showBox('图片码错误');
            }
    ...
    Activity.prototype.getPrize = function(option,callback){
          ...
          var sendType = 'pages';
          var activityCode = option.code || this.option.code;
          var rangeNum = option.range || 0;
          var token = localStorage.getItem("token");   
          var data = '{"token":"'+token+'","activityCode":"'+activityCode+'","rangeNum":"'+rangeNum+'","triggerCode":"'+triggerCode+'","action":"'+triggerAction+'","source":"'+source+'","medium":"'+medium+'","campaign":"'+campaign+'","sendType":"'+sendType+'"}'; 
          var that = this;
