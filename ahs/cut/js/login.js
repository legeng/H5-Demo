...
var urlSpecial='http://mk.aiweixiu.com';
 function urlParams() {
        ...
        $.post(url + "/checkLogin", {
            ...
            if(data.code == 0){
                if(data.body.valid == true){
                	//------------加载页面调用三个接口
	                	if(localStorage.getItem('fifty')==50){
	                		$('.quan .fifty').css('background-image','url(images/50in.png)');
	                	}
	                	if(localStorage.getItem('thirty')==30){
	                		$('.quan .thirty').css('background-image','url(images/30in.png)')
	                	}
	                	if(localStorage.getItem('ten')==10){
	                		$('.quan .ten').css('background-image','url(images/10in.png)');
	                	}
                	//----------------------------
                }else{
    ...
    function login(){
		...
		var phoneJson = '{"mobile":"' + $('#tel').val() + '","verifyCode":"' + verifyCode + '","source":"activity","client":"mobile","type":"popup","activityCode":"ACT_825_JJYJ"}';    		
        $.post(url + "/userLogin", {
...
function fifty(){
        ...
        var sendType='pages';
        var activityCode = 'ACT_825_JJYJ50';
	    var rangeNum ='1';
...
function thirty(){
        ...
        var campaign=urlParams().utm_campaign;
        var sendType='pages';
        var activityCode = 'ACT_825_JJYJ30';
	    var rangeNum ='1';
        var tokenOutRed = localStorage.getItem("token");   
        var data='{"token":"'+tokenOutRed+'","activityCode":"'+activityCode+'","rangeNum":"'+rangeNum+'","trigger":"'+trigger+'","source":"'+source+'","medium":"'+medium+'","campaign":"'+campaign+'","sendType":"'+sendType+'"}'                 	
		$.post(urlSpecial+"/api/activity/common/receiveCoupon", {
