var url = "http://m.mk.aiweixiu.com"+"/api/activity/ahs/common/login";
 function urlParams() {
    ...
    function login(){
		...
		var phoneJson = '{"mobile":"' + $('#tel').val() + '","verifyCode":"' + verifyCode + '","source":"activity","client":"mobile","type":"popup","activityCode":"ACT_722_DEPSS"}';    		
        $.post(url + "/userLogin", {
...
//          		showAlert("登录成功");
                ...
                clearInterval(timer);
                 $("#overlay").remove();
                  ...
                  //----------------------------------------
              	    ...
              	    var sendType='pages';
                    var activityCode = 'ACT_722_DEPSS';
				    var rangeNum ='1';
				    var rangeNum2 ='3';
					...
					$.post("http://mk.aiweixiu.com/api/activity/common/receiveCoupon", {
		       	   ...
		       	   //
		       	   	var data2='{"token":"'+tokenOutRed+'","activityCode":"'+activityCode+'","rangeNum":"'+rangeNum2+'","trigger":"'+trigger+'","source":"'+source+'","medium":"'+medium+'","campaign":"'+campaign+'","sendType":"'+sendType+'"}'                 	
					$.post("http://mk.aiweixiu.com/api/activity/common/receiveCoupon", {
			            "methodName": "receiveCoupon",
			            "timestamp": Date.parse(new Date()),
			            "version": "1.0",
	...
	//login end
    //
    var digitalIndex=1;//能点击
	    	$(".btnBagsUp").on('click',function(e){	    		
	    			//login start
